...
def lin2mat(lin):
    ...
    Returns: 
        mat, torch.Tensor or np.array
    """
    assert len(lin.shape) * (len(lin.shape) - 1) / 2 == lin.shape[
        0], 'Tensor\'s shape should follow [n(n+1)/2, d1, ..., dn].'
...
def lin2mat_batch(lin):
    ...
    Returns: 
        mat, torch.Tensor or np.array
    """
    assert (len(lin.shape) - 1) * (len(lin.shape) - 2) / 2 == lin.shape[
        1], 'Tensor\'s shape should follow [n(n+1)/2, d1, ..., dn].'
    fw, fw_name = get_framework(lin)
    ...
    if fw_name == 'numpy':
        filt_tens = GetNPArrayFromSITK(
            sitk.RecursiveGaussian(sitk.RecursiveGaussian(sitk.RecursiveGaussian(
                GetSITKImageFromNP(tens, True), sigma=sigma, direction=0), sigma=sigma, direction=1), sigma=sigma,
                direction=2), True)
    else:
        filt_tens = torch.from_numpy(GetNPArrayFromSITK(
            sitk.RecursiveGaussian(sitk.RecursiveGaussian(sitk.RecursiveGaussian(
                GetSITKImageFromNP(tens.cpu().numpy(), True), sigma=sigma, direction=0), sigma=sigma, direction=1),
                sigma=sigma, direction=2), True))
    return (filt_tens)
...
def fractional_anisotropy(g):
    ...
    return torch.sqrt(
        3. * (torch.pow((lambd1 - mean), 2) + torch.pow((lambd2 - mean), 2) + torch.pow((lambd3 - mean), 2))) / \
...
def batch_image_interp_2d(x, y, image):
    ...
    for p in range(num_tens):
        if ((math.floor(x[p]) < 0) or (math.ceil(x[p]) >= image.shape[0]) or (math.floor(y[p]) < 0) or (
                math.ceil(y[p]) >= image.shape[1])):
            # data is out of bounds, return identity vector
            scalar[p] = 0.0
            continue
        ...
        if x[p] == math.floor(x[p]) and y[p] == math.floor(y[p]):
            ...
            scalar[p] = abs(x[p] - math.floor(x[p])) * abs(y[p] - math.floor(y[p])) * image[
                math.ceil(x[p]), math.ceil(y[p])] \
                        + abs(x[p] - math.floor(x[p])) * abs(y[p] - math.ceil(y[p])) * image[
                            math.ceil(x[p]), math.floor(y[p])] \
                        + abs(x[p] - math.ceil(x[p])) * abs(y[p] - math.floor(y[p])) * image[
                            math.floor(x[p]), math.ceil(y[p])] \
                        + abs(x[p] - math.ceil(x[p])) * abs(y[p] - math.ceil(y[p])) * image[
                            math.floor(x[p]), math.floor(y[p])]
    return scalar
...
def batch_vect_interp_2d(x, y, vector_field):
    ...
    for p in range(num_tens):
        if ((math.floor(x[p]) < 0) or (math.ceil(x[p]) >= v1.shape[0]) or (math.floor(y[p]) < 0) or (
                math.ceil(y[p]) >= v1.shape[1])):
            # data is out of bounds, return identity vector
            vect[p, 0] = 1
            vect[p, 1] = 1
            continue
        ...
        if x[p] == math.floor(x[p]) and y[p] == math.floor(y[p]):
            ...
            vect[p, 0] = abs(x[p] - math.floor(x[p])) * abs(y[p] - math.floor(y[p])) * v1[
                math.ceil(x[p]), math.ceil(y[p])] \
                         + abs(x[p] - math.floor(x[p])) * abs(y[p] - math.ceil(y[p])) * v1[
                             math.ceil(x[p]), math.floor(y[p])] \
                         + abs(x[p] - math.ceil(x[p])) * abs(y[p] - math.floor(y[p])) * v1[
                             math.floor(x[p]), math.ceil(y[p])] \
                         + abs(x[p] - math.ceil(x[p])) * abs(y[p] - math.ceil(y[p])) * v1[
                             math.floor(x[p]), math.floor(y[p])]
            vect[p, 1] = abs(x[p] - math.floor(x[p])) * abs(y[p] - math.floor(y[p])) * v2[
                math.ceil(x[p]), math.ceil(y[p])] \
                         + abs(x[p] - math.floor(x[p])) * abs(y[p] - math.ceil(y[p])) * v2[
                             math.ceil(x[p]), math.floor(y[p])] \
                         + abs(x[p] - math.ceil(x[p])) * abs(y[p] - math.floor(y[p])) * v2[
                             math.floor(x[p]), math.ceil(y[p])] \
                         + abs(x[p] - math.ceil(x[p])) * abs(y[p] - math.ceil(y[p])) * v2[
                             math.floor(x[p]), math.floor(y[p])]
    return (vect)
...
def tens_interp_2d(x, y, tensor_field):
    ...
    if (math.floor(x) < 0) or (math.ceil(x) >= eps11.shape[0]) or (math.floor(y) < 0) or (
            math.ceil(y) >= eps11.shape[1]):
