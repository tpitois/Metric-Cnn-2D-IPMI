...
def geodesicpath(mode, tensor_lin, vector_lin, mask_image, start_coordinate, initial_velocity, delta_t=0.15,
                 iter_num=18000, filename='', both_directions=False):
    ...
    if both_directions:
        back_x, back_y = geodesicpath(mode, tensor_lin, vector_lin, mask_image, start_coordinate, -init_v, delta_t,
                                      iter_num, filename, both_directions=False)
    ...
    gamma_dot[0] = init_v
    gamma_ddot[0, 0] = -np.einsum('i,i->', gamma_dot[0],
                                  np.einsum('ij,j->i', tens_interp_2d(gamma[0, 0], gamma[0, 1], Gamma1), gamma_dot[0]))
    gamma_ddot[0, 1] = -np.einsum('i,i->', gamma_dot[0],
                                  np.einsum('ij,j->i', tens_interp_2d(gamma[0, 0], gamma[0, 1], Gamma2), gamma_dot[0]))
    ...
    for i in range(2, iter_num):
        ...
        if mode == 'f':
            ...
            gamma_ddot[i - 2, 0] = -np.einsum('i,i->', gamma_dot[i - 2],
                                              np.einsum('ij,j->i', Gamma1_gamma, gamma_dot[i - 2]))
            gamma_ddot[i - 2, 1] = -np.einsum('i,i->', gamma_dot[i - 2],
                                              np.einsum('ij,j->i', Gamma2_gamma, gamma_dot[i - 2]))
...
def batch_geodesicpath_2d(mode, tensor_lin, vector_lin, mask_image, start_coordinates, initial_velocities, delta_t=0.15,
                          iter_num=18000, both_directions=False, Gamma1=None, Gamma2=None):
    ...
    gamma_dot[:, 0] = init_v
    gamma_ddot[:, 0, 0] = -np.einsum('...i,...i->...', gamma_dot[:, 0], np.einsum('...ij,...j->...i',
                                                                                  batch_tens_interp_2d(gamma[:, 0, 0],
                                                                                                       gamma[:, 0, 1],
                                                                                                       Gamma1),
                                                                                  gamma_dot[:, 0]))
    gamma_ddot[:, 0, 1] = -np.einsum('...i,...i->...', gamma_dot[:, 0], np.einsum('...ij,...j->...i',
                                                                                  batch_tens_interp_2d(gamma[:, 0, 0],
                                                                                                       gamma[:, 0, 1],
                                                                                                       Gamma2),
                                                                                  gamma_dot[:, 0]))
    ...
    for i in tqdm(range(2, iter_num)):
        ...
        if mode == 'a':
            ...
            gamma_ddot[:, i - 2, 1] = -np.einsum('...i,...i->...', gamma_dot[:, i - 2],
                                                 np.einsum('...ij,...j->...i', Gamma2_gamma, gamma_dot[:, i - 2]))
            gamma_ddot[:, i - 2] += batch_image_interp_2d(gamma[:, i - 2, 0], gamma[:, i - 2, 1], sigma) * gamma_dot[
                :, i - 2]
            gamma_dot[:, i - 1] = gamma_dot[:, i - 2] + delta_t * (gamma_ddot[:, i - 2])
            gamma[:, i] = gamma[:, i - 1] + delta_t * gamma_dot[:, i - 1]
        ...
        for p in range(num_paths):
            ...
            if (continue_path[p]
                    ...
                    and (mask_image[int(np.ceil(gamma[p, i, 0])), int(
                        np.ceil(gamma[p, i, 1]))] > 0)):  # and gamma[p, i, 0]!=0 and gamma[p, i, 0]!=0:
                active_path = True
...
def geodesicpath_hamilton(mode, tensor_lin, mask_image, start_coordinate, initial_velocity, delta_t=0.15,
                          iter_num=18000, both_directions=False):
    geodesicpath_points_x = np.zeros((iter_num))
    geodesicpath_points_y = np.zeros((iter_num))
    ...
    if both_directions:
        back_x, back_y = geodesicpath_hamilton(mode, tensor_lin, mask_image, start_coordinate, -init_v, delta_t,
                                               iter_num, both_directions=False)
    ...
    for i in range(1, iter_num):
        ...
        if mode == 'i-1':
            p[i, 0] = p[i - 1, 0] - 0.5 * delta_t * np.einsum('i,i->', p[i - 1], np.einsum('ij,j->i', tens_interp_2d(
                gamma[i - 1, 0], gamma[i - 1, 1], d1_tensor_mat), p[i - 1]))
            p[i, 1] = p[i - 1, 1] - 0.5 * delta_t * np.einsum('i,i->', p[i - 1], np.einsum('ij,j->i', tens_interp_2d(
                gamma[i - 1, 0], gamma[i - 1, 1], d2_tensor_mat), p[i - 1]))

        if mode == 'i':
            p[i, 0] = p[i - 1, 0] - 0.5 * delta_t * np.einsum('i,i->', p[i - 1], np.einsum('ij,j->i',
                                                                                           tens_interp_2d(gamma[i, 0],
                                                                                                          gamma[i, 1],
                                                                                                          d1_tensor_mat),
                                                                                           p[i - 1]))
            p[i, 1] = p[i - 1, 1] - 0.5 * delta_t * np.einsum('i,i->', p[i - 1], np.einsum('ij,j->i',
                                                                                           tens_interp_2d(gamma[i, 0],
                                                                                                          gamma[i, 1],
                                                                                                          d2_tensor_mat),
                                                                                           p[i - 1]))
        ...
        if mode == 'mean':
            p[i, 0] = p[i - 1, 0] - 0.5 * delta_t * np.einsum('i,i->', p[i - 1], np.einsum('ij,j->i', tens_interp_2d(
                (gamma[i - 1, 0] + gamma[i, 0]) / 2, (gamma[i - 1, 1] + gamma[i, 1]) / 2, d1_tensor_mat), p[i - 1]))
            p[i, 1] = p[i - 1, 1] - 0.5 * delta_t * np.einsum('i,i->', p[i - 1], np.einsum('ij,j->i', tens_interp_2d(
                (gamma[i - 1, 0] + gamma[i, 0]) / 2, (gamma[i - 1, 1] + gamma[i, 1]) / 2, d2_tensor_mat), p[i - 1]))
...
def geodesic_between_points_torch(tensor_field, mask_image, start_coordinate, end_coordinate, init_velocity=None,
                                  step_size=0.0001, num_iters=18000, filename=''):
    ...
    for it in range(0, num_iters - 1):
        end_point, points_x, points_y = geodesicpath_torch(torch_field, mask, start_coords, init_v[it], delta_t=0.15,
                                                           iter_num=18000, filename='')
