    ...
    'options': {'delta_t':
                ...
                'inputMaskFile':
                    YAMLcfg.Param(default='input_mask.nhdr',
                                  comment='name of input mask file, including path'),
                # include path here because this could be in a different directory than the input tensor
                'outputPathSuffix':
...
class GeodesicShooter(appTypes.BasicApp):
    ...
    def run(self):
        ...
        (x, y, z) = self.run_function(data, mask, self.cfg.options.start_coordinate, self.cfg.options.initial_velocity,
                                      self.cfg.options.delta_t, metric='withoutscaling',
                                      iter_num=self.cfg.options.num_iterations,
                                      filename=f"{self.cfg.paths.outputPrefix}/{self.cfg.options.outputPathSuffix}",
                                      both_directions=self.cfg.options.both_directions)
...
class GeodesicShooter2d(appTypes.BasicApp):
    ...
    def run(self):
        ...
        (x, y) = self.run_function(data, mask, self.cfg.options.start_coordinate, self.cfg.options.initial_velocity,
                                   self.cfg.options.delta_t, metric='withoutscaling',
                                   iter_num=self.cfg.options.num_iterations,
                                   filename=f"{self.cfg.paths.outputPrefix}/{self.cfg.options.outputPathSuffix}")
