...
def show_2d_tensors(img, scale=1, title=None, margin=0.05, dpi=80, has_component_data=False):
    ...
    ellipses = [Ellipse((x, y), width=scaled_evals[x, y, 1], height=scaled_evals[x, y, 0], angle=angles[x, y]) for y in
                range(ysize) for x in range(xsize)]
...
def view_3d_paths(img, paths=None, labels=None, xrng=None, yrng=None, zrng=None, viewer=None, num_tube_pts=500,
                  tube_radius=0.5, colors=[]):
    if xrng is None:
        xrng = [0, img.shape[0]]
    if yrng is None:
        yrng = [0, img.shape[1]]
...
def view_3d_tensors(tens, mask=None, img=None, paths=None, xrng=None, yrng=None, zrng=None, stride=5, scale=None,
                    viewer=None, has_component_data=False, num_tube_pts=500, tube_radius=0.5):
    ...
    if img is not None:
        ...
        sml_img[xrng[0]:xrng[1], yrng[0]:yrng[1], zrng[0]:zrng[1]] = img[
            xrng[0]:xrng[1], yrng[0]:yrng[1], zrng[0]:zrng[1]]
    ...
    for p in range(numpts):
        try:
            if scale is None:
                ...
                ellipses.append(pv.ParametricEllipsoid(evals[p, 2] / evals[p, 2], evals[p, 1] / evals[p, 2],
                                                       evals[p, 0] / evals[p, 2],
                                                       center=tfm_pts[p],
                                                       direction=[evecs[p, 2, 2], evecs[p, 1, 2], evecs[p, 0, 2]]))
            else:
...
def show_tensor_slice(img, slc_no, ax, scale=1, title=None, margin=0.05, dpi=80, has_component_data=False):
    if ax == 0:
        show_3d_tensors(img[slc_no, :, :], scale, title + f", Axis {ax}, Slice {slc_no}", margin, dpi,
                        has_component_data)
    elif ax == 1:
        show_3d_tensors(img[:, slc_no, :], scale, title + f", Axis {ax}, Slice {slc_no}", margin, dpi,
                        has_component_data)
...
def disp_hist_3D(hist, clusterNum, numOfClusters, binSize, slcdim, slc, title='', margin=0.05, dpi=80):
    ...
    if slcdim == 0:
        t = ax.imshow(hist[clusterNum::numOfClusters].reshape(binSize, binSize, binSize)[slc, :, :], extent=extent,
                      interpolation=None)
    elif slcdim == 1:
        t = ax.imshow(hist[clusterNum::numOfClusters].reshape(binSize, binSize, binSize)[:, slc, :], extent=extent,
                      interpolation=None)
...
def vis_tensors(tensor_field, title, save_file=False, filename='', mask=None, scale=0.3, opacity=0.5,
                show_axis_labels=True, ax=None, zorder=1, stride=None):
    eps11 = tensor_field[0, :, :]
    eps12 = tensor_field[1, :, :]
    eps22 = tensor_field[2, :, :]
    # visualizing ellipses
    ...
    if mask is None:
        ells = [Ellipse((x, y), width=scaled_evals[x, y, 1], height=scaled_evals[x, y, 0], angle=angles[x, y],
                        zorder=zorder) for y in range(eps11.shape[1]) for x in range(eps11.shape[0])]
    else:
        for x in range(0, eps11.shape[0], stride):
            for y in range(0, eps11.shape[1], stride):
                if mask[x, y]:
                    ells.append(
                        Ellipse((x, y), width=scaled_evals[x, y, 1], height=scaled_evals[x, y, 0], angle=angles[x, y],
                                zorder=zorder))
...
def vis_path(points_x, points_y, fig=None, label='', color='b', sz=20, alpha=1, save_file=False, filename='',
             yscale='linear', ax=None, zorder=1, stride=None, show_legend=True):
    if ax is None:
        ax = plt.gca()
    if stride is None:
        stride = 1
...
def gen_and_vis_paths(file_pattern, start_iter, stop_iter, inc_iter, start_coords, interp_colors, geo_iters,
                      geo_delta_t,
...
def gen_and_vis_paths_per_coords(tens, start_coords, interp_colors, geo_iters, geo_delta_t,
    ...
    for it in range(len(start_coords)):
        ...
        vis_path(geox, geoy, tens_fig, legend_pattern.format(start_coords[it]), interp_colors[idx], line_width, 1,
                 False)
