...
def testCubic2Tens2D(xsz, ysz, out_prefix, xrg=None, yrg=None, zero_padding_width=None):
    ...
    return (testGenAndWriteCubic(xsz, ysz, a0_2, a1_2, a2_2, a3_2, b0_2, b1_2, b2_2, b3_2, out_prefix, xrg, yrg,
                                 zero_padding_width))


def testGenAndWriteCubic(xsz, ysz, a0, a1, a2, a3, b0, b1, b2, b3, out_prefix, xrg=None, yrg=None,
    ...
    dc2 = lambda t, dt: data.gen.d_cubic(t, b3, b2, b1, b0, dt)
    # TODO ok if xrg, yrg not isotropic?
    (cubic_no_blur_img, cubic_no_blur_ten, cubic_no_blur_seed, cubic_no_blur_xrg,
     cubic_no_blur_yrg) = data.gen.gen_2D_tensor_image(xsz, ysz, 0, 1, 1000, c1, dc1, c2, dc2, 1 / 5.0, 15, 6.0, 0.05,
                                                       0.95, True, False, False, xrg, yrg,
...
def testRandomCubicTens2D(xsz, ysz, out_prefix, num_cubics=None, xrg=None, yrg=None, zero_padding_width=None):
    ...
    if num_cubics is None:
        num_cubics = 2
    if num_cubics >= 1:
        xrg, yrg = testGenAndWriteCubic(xsz, ysz, a0, a1, a2, a3, b0, b1, b2, b3, out_prefix + "cubic1_", xrg, yrg,
                                        zero_padding_width)
    else:
        return

    if num_cubics >= 2:
        testGenAndWriteCubic(xsz, ysz, a0_2, a1_2, a2_2, a3_2, b0_2, b1_2, b2_2, b3_2, out_prefix + "cubic2_", xrg, yrg,
                             zero_padding_width)
    else:
        return
    ...
    for cc in range(3, num_cubics + 1):
        ...
        # b0_r = 0.0
        testGenAndWriteCubic(xsz, ysz, a0_r, a1_r, a2_r, a3_r, b0_r, b1_r, b2_r, b3_r, out_prefix + f"cubic{cc}_", xrg,
                             yrg, zero_padding_width)
    return (xrg, yrg)


def testVaryRectangleTens2D(xsz, ysz, out_prefix):
    tens = data.gen.gen_2D_rectangle_gradient_ratio(xsz, ysz, 1 / 6.0, rotation=0, do_isotropic=True,
...
"""
    ...
    if do_3d:
        ...
        testRandomCubicTens2D(int(options.xsz), int(options.ysz), options.outdir + "/metpy_", int(options.numcubics),
                              cubic_xrg, cubic_yrg, 4)
