...
def run_simulation(sim_name, input_tensor, input_mask, input_dir, output_root_dir, min_eigenvalues, num_iters, sigmas,
        ...
        for num_iter in num_iters:
            for sigma in sigmas:
                ...
                try:
                    ...
                    print(
                        f"Error {e} while running {test_case} with config {YAMLcfg.ConfigToYAML(me.MetricEstimatorConfigSpec, me_cfg)}")
                    success = False
                ...
                for coords in start_coords:
                    for init_velocity in init_velocities:
                        ...
                        try:
                            ...
                            print(
                                f"Error {e} while geodesic shooting for {test_case} with config {YAMLcfg.ConfigToYAML(gs.geodesicShootingConfigSpec, gs_cfg)}")
                            success = False
                        ...
                        try:
                            ...
                            print(
                                f"Error {e} during Euler integration for {test_case} with config {YAMLcfg.ConfigToYAML(gs.geodesicShootingConfigSpec, gs_cfg)}")
                            success = False
...
def summarize_results(results, sim_name, output_root_dir, subplot_nrows, subplot_ncols,
    ...
    colors = ['k', 'tab:blue', 'tab:orange', 'tab:purple', 'tab:green', 'tab:red', 'tab:brown', 'tab:pink', 'tab:gray',
              'tab:olive', 'tab:cyan']
    coloridx = 1
    case_count = 1
    ...
    scalar_files = ['divergence_rhs', 'rhs', 'output_result_rhs', 'output_alpha_final', 'output_result_lhs',
                    'usermult2_lhs']
    ...
    for test in results:
        ...
        if show_energy_plot:
            ...
            vis_path(energies['KSP Residual norm']['iter'], energies['KSP Residual norm']['value'], resid_fig, label,
                     colors[coloridx], 5, 1, False, yscale='log')
            vis_path(energies['current energy']['iter'], energies['current energy']['value'], energy_fig, label,
                     colors[coloridx], 5, 1, False, yscale='log')
            vis_path(-np.log(energies['KSP Residual norm']['value']), energies['current energy']['value'],
                     energy_resid_fig, label, colors[coloridx], 5, 1, False, yscale='log')
        ...
        for path in test["paths"]:
            ...
            if path["shooting"]["status"]:
                vis_path(path["shooting"]["x"], path["shooting"]["y"], tens_fig, label + "(Geodesic)", colors[coloridx],
                         20, 1, False)
