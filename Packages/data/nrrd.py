...
class nrrdTransform():
    ...
    def set_nrrd_space_directions_from_spacing(self):
        ...
        if self.file:
            ...
            for dim in self.spacing:
                ...
                spacing_str += ")"
            cmds.append(
                'cat %s | sed s/"space directions.*"/"space directions: %s"/ > %s' % (self.file, spacing_str, tempfile))
            cmds.append('cat %s > %s' % (tempfile, self.file))
            cmds.append('rm %s' % (tempfile))
            for cmd in cmds:
                try:
    ...
    def set_nrrd_space_directions(self):
        ...
        if self.file:
            ...
            cmds = []
            cmds.append(
                'cat %s | sed s/"space directions.*"/"space directions: %s"/ > %s' % (self.file, self.space_directions,
                                                                                      tempfile))
            cmds.append('cat %s > %s' % (tempfile, self.file))
            cmds.append('rm %s' % (tempfile))
            for cmd in cmds:
    ...
    def add_nrrd_space_dimension(self):
        ...
        if self.file:
            ...
            tempfile = fileroot + "_temp" + fileext
            cmds = []
            cmds.append('cat %s | sed 5s/"^//p; 5s/^.*/space dimension: %s"/ > %s' % (self.file, self.space_dimension,
                                                                                      tempfile))
            cmds.append('cat %s > %s' % (tempfile, self.file))
            cmds.append('rm %s' % (tempfile))
            for cmd in cmds:
    ...
    def write_gradients(self):
        ...
        if self.file:
            cmds = []
            cmds.append(
                """cat %s | sed '/DWMRI_gradient.*/ '!'d' | sed s/"DWMRI_gradient_00.*:= "/""/ > %s""" % (self.file,
                                                                                                          self.gradientFile))
            for cmd in cmds:
                print(cmd)
                try:
