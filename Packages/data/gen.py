    ...
    for xbin in ds.keys():
        for ybin in ds[xbin].keys():
            ...
            for entry in ds[xbin][ybin]:
                pixsum += entry[
                    2]  # anti aliasing, by adding percent of pixel coverage instead of hard setting to 1 or 0
                cnt += 1
            if cnt > 0:
...
def add_to_3D_seed_image(im, x, y, zs, dx, dy, rsq, xrg, yrg, zrg, seed1, seed2):
    ...
    for zz in zs:
        ...
        for tt in tspc:
            pts, pcts = get_3D_antialiased_points((x[seed1] + tt * dy[seed1] - xrg[0]) / xinc,
                                                  (y[seed1] - tt * dx[seed1] - yrg[0]) / yinc, (zz - zrg[0]) / zinc)
        ...
        tspc = np.linspace(-max_t, max_t, 100)
        for tt in tspc:
            pts, pcts = get_3D_antialiased_points((x[seed2] + tt * dy[seed2] - xrg[0]) / xinc,
                                                  (y[seed2] - tt * dx[seed2] - yrg[0]) / yinc, (zz - zrg[0]) / zinc)
            for pt, pct in zip(pts, pcts):
                xbin = pt[0]
                ybin = pt[1]
                zbin = pt[2]
...
def gen_2D_tensor_image(xsz, ysz, tmin, tmax, numt, xf, dxf, yf, dyf, fdist, numf, ratio, seed_t1, seed_t2,
                        do_isotropic=True, do_blurring=True, do_matching_range=False, xrng=None, yrng=None,
                        template_tensor=None, zero_padding_width=None):
        ...
        for idx in range(xys.shape[-1]):
            ...
            for pt, pct in zip(pts, pcts):
                ...
                if ybin not in ds[xbin].keys():
                    ds[xbin][ybin] = []
                ds[xbin][ybin].append(
                    [c, idx, pct])  # c for which curve it is, pct for the amount of function value to include

    add_to_2D_image(img, xys, dxdys, ds)
    add_to_2D_tensor_image(tens, xys, dxdys, ds, ratio,
                           template_tensor)  # need to figure out blurring in this new context
...
def gen_3D_tensor_image(xsz, ysz, tmin, tmax, numt, xf, dxf, yf, dyf, fdist, numf, ratio, seed_t1, seed_t2,
                        do_isotropic=True, do_blurring=True, do_matching_range=False, xrng=None, yrng=None, zrng=None,
                        template_tensor=None, zero_padding_width=None):
    ...
    for zz in zs:
        ...
        for idx in range(len(x)):
            ...
            for tt in tspc:
                pts, pcts = get_3D_antialiased_points((x[idx] + tt * dy[idx] - xrg[0]) / xinc,
                                                      (y[idx] - tt * dx[idx] - yrg[0]) / yinc, (zz - zrg[0]) / zinc)
                for pt, pct in zip(pts, pcts):
                    xbin = pt[0]
                    ybin = pt[1]
                    zbin = pt[2]
    ...
    add_to_3D_tensor_image(tens, xys, dxim, dyim, pixsum, cnts, ratio,
                           template_tensor)  # need to figure out blurring in this new context
...
def gen_2D_annulus(xsz, ysz, ratio, do_isotropic=True, do_blurring=True,
    ...
    return (
        gen_2D_tensor_image(xsz, ysz, 0, np.pi, 1000, cos_t, d_cos_t, sin_t, d_sin_t, 1 / 5.0, 15, ratio, 0.05, 0.95,
                            do_isotropic, do_blurring, do_matching_range, xrng=xrng, yrng=yrng,
...
def gen_3D_annulus(xsz, ysz, ratio, do_isotropic=True, do_blurring=True,
                   do_matching_range=False, xrng=None, yrng=None, zrng=None, template_tensor=None,
                   zero_padding_width=None):
    return (
        gen_3D_tensor_image(xsz, ysz, 0, np.pi, 1000, cos_t, d_cos_t, sin_t, d_sin_t, 1 / 5.0, 15, ratio, 0.05, 0.95,
                            do_isotropic, do_blurring, do_matching_range, xrng=xrng, yrng=yrng, zrng=zrng,
                            template_tensor=template_tensor, zero_padding_width=zero_padding_width))
...
def gen_2D_rectangle_constant_ratio(xsz, ysz, ratio, rotation=None, do_isotropic=True, zero_padding_width=None):
    ...
    if rotation:
        ...
        rect_tens[zero_padding_width:-zero_padding_width, zero_padding_width:-zero_padding_width] = \
        np.matmul(const_R, np.matmul(tens, np.transpose(const_R)))[triu_idx]
